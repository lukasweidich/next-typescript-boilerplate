...
export const logIn = (username: string, password: string) => async (dispatch): Promise<void> => {
    const { token, user } = await authenticateWithUsernameAndPassword(username, password);
